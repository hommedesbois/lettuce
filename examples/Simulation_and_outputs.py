...
# ---------- Set up simulation -------------
device = torch.device("cpu")  # replace with device("cpu"), if no GPU is available
...
kinE_reporter = lt.ObservableReporter(lt.IncompressibleKineticEnergy(lattice, flow), interval=10)
...
ax1.plot(simulation.flow.units.convert_time_to_pu(range(0, E.shape[1])), E[:, 2])
